...
# ------------------------- Train a 196-100-10 Network with 10000 Images with Initialization Xavier_relu! --------------
...
#         plt.show()
        ...
        mnist_net_single.save_params('weights_after_test_7')
    ...
    def write_time(self, start, end, start_time):
        title = 'times_for_mnist_net_7_' + self.time_stamp + '.txt'
    ...
    def write_acc(self, list):
        title = 'accuracy_for_mnist_net_7_' + self.time_stamp + '.txt'
        with open(title, 'a') as f:
            f.write(str(list) + '\n')
