...
#         plt.show()
        ...
        mnist_net_single = network_class.Network([(196), (100), (10)],  weights=None, bias=None, activation_function="sigmoid", initilizer="xavier_tf", dropout=0.0)
    ...
    def write_time(self, start, end, start_time):
        title = 'times_for_mnist_net_8_' + self.time_stamp + '.txt'
    ...
    def write_acc(self, list):
        title = 'accuracy_for_mnist_net_8_' + self.time_stamp + '.txt'
        with open(title, 'a') as f:
            f.write(str(list) + '\n')
